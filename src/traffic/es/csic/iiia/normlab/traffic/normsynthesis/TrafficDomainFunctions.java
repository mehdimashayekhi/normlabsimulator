...
import es.csic.iiia.normlab.traffic.config.Gconfc;
...
public class TrafficDomainFunctions implements DomainFunctions {
	...
	public EnvironmentAgentContext agentContextFunction(long carId, View view) {
		...
		//outofzone=carContextFactory.outofintersectionzone(tView, carId);
	...
	@Override
	public EnvironmentAgentContext agentContextFunctionconf(long carId, View view, List<Long> ids) {
		TrafficView tView = (TrafficView)view;
		//outofzone=carContextFactory.outofintersectionzone(tView, carId);
		CarContext context = carContextFactory.getCarContextInconf(tView,
				carId, CarContext.Type.Front, ids);

		return context;
	}
	
	@Override
	public EnvironmentAgentContext agentContextFunctioncol(long carId, View view) {
		TrafficView tView = (TrafficView)view;
		//outofzone=carContextFactory.outofintersectionzone(tView, carId);
		CarContext context = carContextFactory.getCarContextIncol(tView,
				carId, CarContext.Type.Front);
	...
	@Override
	public EnvironmentAgentContext agentContextFunctionmod(long carId, View view, List<Long> ids) {
		TrafficView tView = (TrafficView)view;
		//outofzone=carContextFactory.outofintersectionzone(tView, carId);
		CarContext context = carContextFactory.getCarContextInmod(tView,
				carId, CarContext.Type.Front, ids);
	...
	public List<Conflict> getConflicts(Goal goal,	ViewTransition vTrans) {
		...
		View viewTimeTminus = vTrans.getView(-1);
		TrafficView tfViewm = (TrafficView) viewTimeTminus;
			...
			for(int x=0; x<numElems; x++) {
				...
				if(TrafficStateManager.getType(desc) == StateType.Collision ||
					...
					if(resolvable) {
  					...
  					/* Generate new conflict, adding the conflicting agent */
						for (Long id: ids){
							outofzone=carContextFactory.outofintersectionzone(tfViewm, id);
							if (outofzone){
								conflictingAgents.add(id);
							}
						}
		...
		//adding the other goal which is conflicting, this was added by mehdi
		if(goal instanceof Gconfc) {
			int numElems = tfView.getNumElements();
			List<Long> conflictingAgents = new ArrayList<Long>();
			for(int x=0; x<numElems; x++) {
				String desc = tfView.get(x);

				if(TrafficStateManager.getType(desc) == StateType.Conflict) {					
					conflicting=true;
					List<Long> ids = TrafficStateManager.getCarIds(desc);
					//conflictingAgents.addAll(ids);
					Conflict conflict = new Conflict(vTrans.getSensor(), viewTimeT,
