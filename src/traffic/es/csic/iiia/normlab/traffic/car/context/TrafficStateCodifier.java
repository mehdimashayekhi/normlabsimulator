...
{	
	...
	public static String codify(TrafficElement element)	{
		...
		else if(element instanceof Conflicting)	{
			desc = TrafficStateManager.CONFLICT + codifyConflict((Conflicting)element);
		}
...
//		}
	...
	/**
	 * 
	 * @return// note by mehdi: I should come back to this and modify this method right now it is like collision
	 */
	private static String codifyConflict(Conflicting con) {
		int numElems = con.getElements().size();
		String state = "";
		...
		// Codify each car
		for(TrafficElement elem : con.getElements()) {
			Car car = (Car)elem;
			state += codifyCar(car);
		}			
		return state;
	 ...
	 * @return
	 */
	private static String decodifyConflict(String codDesc) {
		return "+";
	}
