...
public class Car implements TrafficElement, EnvironmentAgent
{
	...
	protected CarPosition oldposition; 
	...
	{
		CarMap carMap = TrafficSimulator.getMap();
		// Execute Go action and turn if needed
		this.executef();
		this.turnf(carMap);
	}
	...
	public void perceiveAndReasonmod(List<Long> conflictingids) {
		CarMap carMap = TrafficSimulator.getMap();

		// Perceive and reason to get the action to apply in the next step
		this.context = this.perceivemod(carMap, conflictingids);
		//System.out.println(context.toString());
		this.nextAction = this.reason();
	}
	...
	public void modifyAction() {
		...
		this.nextAction = CarAction.Stop;
	}
	...
	private CarContext perceivemod(CarMap carMap, List<Long> conflictingids) {
		return this.generateScopemod(carMap, conflictingids);
	}
    ...
    //reasoner.update(context, action);
	...
	public void rewarding(double reward){
		reasoner.update(this, context, this.getNextAction(), reward);
	}
	...
	private void movementb() {
		position = position.addb(getMovementVectorb(), oldposition.getDirection());
	}
	 ...
	 * Check if has to turn and turn if so. Turns the car to the direction it has to turn
	 */
	public void turnf(CarMap carMap) {
		int x = position.getX();
		int y = position.getY();
	 ...
	 * Calculates the movement vector of the car
	 */
	private GridPoint calcMovementVectorb() {
		int movement[] = Utilities.getDirVector(oldposition.getDirection());
		for(int i=0; i<movement.length; i++)
			movement[i] *= speed;
		return new GridPoint(movement[0],movement[1]);
	}
	...
	public CarContext generateScopemod(CarMap carMap, List<Long> conflictingids) {
		CarContext context = contextFactory.getCarContextInmod(carMap, id, CarContext.Type.Front,conflictingids);
		return context;
	}
	 ...
	 * @return
	 */
	private GridPoint getMovementVectorb(){
		return calcMovementVectorb();
	}
